...
pub fn make_secret_key(
    ...
    let mut command = Command::new("/usr/local/bin/tlcs-ab/invert4blockchain.sh");
    ...
    println!("SS: {:?}", String::from_utf8_lossy(&secret_key));
...
mod tests {
    ...
    fn invert_works() {
        let mut all_participant_data = generate_participant_data(2);
        let mut participant_data_2 = generate_participant_data(2);
        //println!("PD1: {}", hex::encode(all_participant_data.clone()));
        //println!("PD2: {}", hex::encode(participant_data_2.clone()));
        all_participant_data.append(&mut participant_data_2);
        let public_key = aggregate_participant_data(2, all_participant_data.clone());
        //println!("AG: {:?}", String::from_utf8_lossy(&public_key));
