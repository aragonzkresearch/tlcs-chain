...
pub async fn get_contributions_by_round(client: HttpClient, round: u32) -> Result<String> {
    ...
    let res = client
        .abci_query(
            Some("/azkr.tlcs.v1beta1.Query/AllContributionsByRound"
                    .parse()
                    .expect("hard coded path will always succeed"),
            ),
            query.encode_vec(),
...
pub async fn get_contributions_by_round_and_scheme(client: HttpClient, round: u32, scheme: u32) -> Result<String> {
    ...
    let res = client
        .abci_query(
            Some("/azkr.tlcs.v1beta1.Query/AllContributionsByRoundAndScheme"
                    .parse()
                    .expect("hard coded path will always succeed"),
            ),
            query.encode_vec(),
...
pub async fn get_keypairs_by_round(client: HttpClient, round: u32) -> Result<String> {
    ...
    let res = client
        .abci_query(
            Some("/azkr.tlcs.v1beta1.Query/AllKeyPairsByRound"
                    .parse()
                    .expect("hard coded path will always succeed"),
            ),
            query.encode_vec(),
...
pub async fn get_loe_data_by_round(client: HttpClient, round: u32) -> Result<String> {
    ...
    let res = client
        .abci_query(
            Some("/azkr.tlcs.v1beta1.Query/AllLoeDataByRound"
                    .parse()
                    .expect("hard coded path will always succeed"),
            ),
            query.encode_vec(),
